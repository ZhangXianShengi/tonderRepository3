...
public class AuthController {
    ...
    public ResponseEntity<Void> login(@RequestParam("username")String username, @RequestParam("password")String password,
        ...
        //1.远程调用用户中心微服务查询用户中心，并生成jwt
        String token = this.authService.login(username,password);
        if (StringUtils.isBlank(token)){
            return ResponseEntity.badRequest().build();
        }

        //2.使用CookieUtils的setCookie方法，就可以吧jwt类型的token设置到；游览器的cookie
     ...
     */
    @GetMapping("verify")
    public ResponseEntity<UserInfo> verify(@CookieValue("LEYOU_TOKEN") String token,
                                           HttpServletRequest request, HttpServletResponse response){
        try {
