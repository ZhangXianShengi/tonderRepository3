<template>
  <v-form v-model="valid" ref="brandForm">
    ...
    <v-layout row>
      ...
      <v-flex>
        <v-upload
          v-model="oldBrand.image" url="/upload/image" :multiple="false" :pic-width="250" :pic-height="90"
        />
      </v-flex>
    </v-layout>
    <v-layout class="my-4">
      ...
      submit() {
        ...
        if (this.$refs.brandForm.validate()) {
          ...
          this.$http({
            ...
            url:'/item/brand',
            data:this.$qs.stringify(params) //将json转为普通的字符串
          }).then(()=>{
            //关闭窗口
            this.$emit("close");
